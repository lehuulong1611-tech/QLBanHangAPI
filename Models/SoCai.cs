using System;
using System.Collections.Generic;

namespace QLBanHangAPI.Models;

public partial class SoCai
{
    public string LoaiCt { get; set; } = null!;

    public string ChungTu { get; set; } = null!;

    public DateTime? Ngay { get; set; }

    public string Nhanvien { get; set; } = null!;

    public string KhachHang { get; set; } = null!;

    public string Mst { get; set; } = null!;

    public string NguoiMua { get; set; } = null!;

    public string Diachi { get; set; } = null!;

    public string? Ghichu { get; set; }

    public string Kho { get; set; } = null!;

    public decimal ChietKhau { get; set; }

    public decimal KhuyenMai { get; set; }

    public decimal TienThue { get; set; }

    public decimal TienHang { get; set; }

    public decimal? PsNo { get; set; }

    public decimal? PsCo { get; set; }

    public decimal ThanhTien { get; set; }

    public decimal PhaiTra { get; set; }

    public decimal ThanhToan { get; set; }

    public decimal ConLai { get; set; }

    public decimal TinhTrang { get; set; }

    public DateTime? NgayHenTra { get; set; }

    public string The { get; set; } = null!;

    public short? Gio { get; set; }

    public decimal MucThue { get; set; }

    public string? SoHoaDon { get; set; }

    public string? KhoanMuc { get; set; }

    public bool KhoaSo { get; set; }

    public string? PhieuDatHang { get; set; }

    public string? NganHang { get; set; }

    public DateTime? NgayHoaDon { get; set; }

    public DateTime? NgayNhanHang { get; set; }

    public DateTime? NgayNhanHoaDon { get; set; }

    public string? PhieuGiaoHang { get; set; }

    public string? DiaDiemBanGiao { get; set; }

    public string? HinhThucThanhToan { get; set; }

    public string? DoiTuong { get; set; }

    public string? SoHopDong { get; set; }

    public decimal? ChiPhiNoi { get; set; }

    public decimal? ChiPhiNgoai { get; set; }

    public DateTime? NgayNopThue { get; set; }

    public string? TienTe { get; set; }

    public decimal? TyGia { get; set; }

    public string? GhiChuDonVi { get; set; }

    public string? LoaiTien { get; set; }

    public decimal? KhuyenMaiHoaDon { get; set; }

    public short? User1 { get; set; }

    public DateTime? Date1 { get; set; }

    public short? User2 { get; set; }

    public DateTime? Date2 { get; set; }

    public short? IdcuaHang { get; set; }

    public string? SqlDetail { get; set; }

    public int Id { get; set; }

    public string? NhanVien1 { get; set; }

    public decimal? LePhi { get; set; }
}
